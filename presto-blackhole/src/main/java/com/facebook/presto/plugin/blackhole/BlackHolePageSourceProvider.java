        ...
        implements ConnectorPageSourceProvider
{
            ...
            List<ColumnHandle> columns)
    {
        ...
        return new BlackHolePageSource(pages, blackHoleSplit.getPageProcessingDelay());
    }
    ...
    private Block createZeroBlock(Type type, int rowsCount, Slice characters)
    {
        ...
        if (isVarcharType(type)) {
            slice = characters.slice(0, Math.min(((VarcharType) type).getLength(), characters.length()));
        }
    ...
    {
        return ImmutableSet.of(TINYINT, SMALLINT, INTEGER, BIGINT, DOUBLE, BOOLEAN, DATE, TIMESTAMP, VARBINARY).contains(type)
