        ...
        extends AstVisitor<Node, C>
{
    ...
    {
        Expression operand = (Expression) process(node.getOperand(), context);
        List<WhenClause> whenClauses = node.getWhenClauses().stream().map(value -> (WhenClause) process(value, context)).collect(toImmutableList());
    ...
    {
        return new AtTimeZone(node.getLocation(), (Expression) process(node.getValue(), context), (Expression) process(node.getTimeZone(), context));
    }
}
