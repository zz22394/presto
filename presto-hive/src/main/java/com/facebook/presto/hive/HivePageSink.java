        ...
        implements ConnectorPageSink
{
            ...
            HdfsEnvironment hdfsEnvironment)
    {
        ...
        conf = new JobConf(hdfsEnvironment.getConfiguration(writePath));
    }
    ...
    private static class HiveRecordWriter
    {
                ...
                JobConf conf)
        {
            ...
            if (!missingColumns.isEmpty()) {
                throw new PrestoException(HIVE_WRITER_ERROR, format("Partition %s does not have columns %s", inputColumnNames, missingColumns));
            }
            if (fileColumnNames.size() != fileColumnTypes.size()) {
                throw new PrestoException(HIVE_INVALID_METADATA, format("Table '%s.%s' has metadata for column names of types",
                        schemaName,
                        tableName));
            }
            ...
            tableInspector = getStandardStructObjectInspector(fileColumnNames, HiveWriteUtils.getJavaObjectInspectors(fileColumnTypes));
