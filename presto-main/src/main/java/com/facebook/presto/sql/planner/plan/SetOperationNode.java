        ...
        extends PlanNode
{
            ...
            @JsonProperty("outputs") List<Symbol> outputs)
    {
        ...
        for (Collection<Symbol> symbols : this.symbolMapping.asMap().values()) {
            checkArgument(symbols.size() == this.sources.size(), "Every source needs to map its symbols to an output %s operation symbol", getSetOperationType().name());
        }
    ...
    public abstract SetOperation.SetOperationType getSetOperationType();
}
