...
public final class PlanSanityChecker
{
            ...
            extends PlanVisitor<Void, Void>
    {
        ...
        private Void visitSetOperation(SetOperationNode node, Void context)
        {
            for (int i = 0; i < node.getSources().size(); i++) {
                PlanNode subplan = node.getSources().get(i);
                checkDependencies(subplan.getOutputSymbols(), node.sourceOutputLayout(i), "%s subplan must provide all of the necessary symbols", node.getSetOperationType().name());
                subplan.accept(this, context); // visit child
            }
