            ...
            extends SignatureBinder
    {
        ...
        public Optional<Signature> bind(List<? extends Type> actualArgumentTypes)
        {
            ...
            for (Type type : actualArgumentTypes) {
                ...
                boundTypesSignatures.add(varcharType(paramLength).getTypeSignature());
            }
            ...
            return Optional.of(
                    new Signature(
                            ...
                            varcharType(lengthAccumulator).getTypeSignature(),
                            boundTypesSignatures.build()));
        }

        private static Type varcharType(long length)
        {
            if (length == VarcharType.UNBOUNDED_LENGTH) {
                return createUnboundedVarcharType();
            }
            else {
                return createVarcharType(Ints.saturatedCast(length));
            }
        }
