...
public class BindableAggregationFunction
{
    ...
    private static List<AggregationMetadata.ParameterMetadata> getParameterMetadata(@Nullable Method method, List<Type> inputTypes)
    {
        ...
        ImmutableList.Builder<AggregationMetadata.ParameterMetadata> builder = ImmutableList.builder();
        ...
        for (int i = 1; i < annotations.length; i++) {
            ...
            if (baseTypeAnnotation instanceof SqlType) {
                builder.add(AggregationMetadata.ParameterMetadata.fromSqlType(inputTypes.get(i - 1), AggregationCompiler.isParameterBlock(annotations[i]), AggregationCompiler.isParameterNullable(annotations[i]), methodName));
            }
                ...
                builder.add(new AggregationMetadata.ParameterMetadata(SAMPLE_WEIGHT));
            }
            else {
