        ...
        implements ConnectorMetadata
{
    ...
    public TpchTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName)
    {
        ...
        return new TpchTableHandle(connectorId, tableName.getTableName(), schemaParameters.get().scaleFactor, tableName.getSchemaName(), schemaParameters.get().useDecimal);
    }
    ...
    public ConnectorTableMetadata getTableMetadata(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        return getTableMetadata(schemaName, tpchTable, tpchTableHandle.isUseDecimal());
    }

    private static ConnectorTableMetadata getTableMetadata(String schemaName, TpchTable<?> tpchTable, boolean useDecimal)
    {
        ImmutableList.Builder<ColumnMetadata> columns = ImmutableList.builder();
        for (TpchColumn<? extends TpchEntity> column : tpchTable.getColumns()) {
            columns.add(new ColumnMetadata(column.getColumnName(), getPrestoType(column.getType(), useDecimal)));
        }
        ...
        for (String schemaName : getSchemaNames(prefix.getSchemaName())) {
            for (TpchTable<?> tpchTable : TpchTable.getTables()) {
                if (prefix.getTableName() == null || tpchTable.getTableName().equals(prefix.getTableName())) {
                    ...
                    ConnectorTableMetadata tableMetadata = getTableMetadata(schemaName, tpchTable, schemaParameters.get().useDecimal);
    ...
    public static Type getPrestoType(TpchColumnType tpchType, boolean useDecimal)
    {
        switch (tpchType) {
            case BIGINT:
                return BIGINT;
    ...
    private static class SchemaParameters
    {
        ...
        SchemaParameters(double scaleFactor, boolean useDecimal)
        {
