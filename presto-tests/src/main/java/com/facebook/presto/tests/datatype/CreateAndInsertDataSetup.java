        ...
        implements DataSetup
{
    ...
    {
        String valueLiterals = Joiner.on(", ").join(inputs.stream().map(DataTypeTest.Input::toLiteral).iterator());
    ...
    {
        List<String> columnTypeDefinitions = inputs.stream().map(DataTypeTest.Input::getInsertType).collect(toList());
