 ...
 * **Type:** ``String`` (data size)
 * **Default value:** ``8 MB``
 * **Description:** Serves as default value for ``orc_stream_buffer_size`` session property defining max size of ORC streaming operators. Higher value allows to load and process bigger chunks of data possibly increasing latency and decreasing parallelism but also increasing processing efficiency.
